#!/usr/bin/env python3
"""
Calculate technical potential (MW) per zone for ALL generation technologies.

Converts availability fractions (from unified availability matrix) and
available areas (offshore KRA) into maximum installable capacity (p_nom_max)
using technology-specific capacity densities from config.

Onshore technologies (onwind, solar, smr, electrolysis, h2_turbine):
...
Input:
    - resources/land/availability_matrix_{network_model}.csv — unified
      availability matrix with all onshore technologies
    ...
    - Config: capacity densities from defaults.yaml and nuclear/hydrogen config

Output:
    - resources/land/technical_potential_{network_model}.csv —
      combined onshore + offshore potential with all attributes

Author: K O'Neill
Date: 2026-03-23
Updated: 2026-03-29 — renamed from calculate_renewable_potential, extended
    to handle all technologies via unified availability matrix
"""
...
log_path = (
    ...
    else "calculate_technical_potential"
)
...
def calculate_onshore_potential(avail_df, zone_areas, config, nuclear_config, hydrogen_config):
    """Calculate onshore technical potential per zone per technology.

    Parameters
    ----------
    avail_df : pd.DataFrame
        Availability matrix with zone index and technology columns
        (onwind, solar, smr, electrolysis, h2_turbine, etc.)
        containing fractions 0.0–1.0.
    zone_areas : pd.Series
        Zone areas in km², indexed by zone name.
    config : dict
        Land constraints config from defaults.yaml.
    nuclear_config : dict
        Nuclear siting constraints config from defaults.yaml.
    hydrogen_config : dict
        Hydrogen siting constraints config from defaults.yaml.
    ...
    """
    rows = []
    # Process all non-zone columns in the availability matrix
    technologies = [col for col in avail_df.columns if col != "zone"]

    for tech in technologies:
        # Route to correct config section based on technology
        if tech == "smr":
            density = nuclear_config.get("smr", {}).get("capacity_density", 1.0)
        elif tech in ("electrolysis", "h2_turbine"):
            density = hydrogen_config.get(tech, {}).get("capacity_density", 1.0)
        else:
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("CALCULATE TECHNICAL POTENTIAL")
    logger.info("=" * 80)

    # Get parameters
    if snk:
        avail_matrix_path = snk.input.availability_matrix
        ...
        hydrogen_config = snk.params.hydrogen_config
    else:
        avail_matrix_path = "resources/land/availability_matrix_Zonal.csv"
        offshore_path = "resources/land/offshore_available_kra_Zonal.gpkg"
        output_path = "resources/land/technical_potential_Zonal.csv"
        ...
        if defaults_path.exists():
            with open(defaults_path) as f:
                ...
                config = defaults.get("land_constraints", {})
                nuclear_config = defaults.get("nuclear", {}).get("siting_constraints", {})
                hydrogen_config = defaults.get("hydrogen", {}).get("siting_constraints", {})
        else:
            config = {}
            nuclear_config = {}
            hydrogen_config = {}
    ...
    for name, path in [
        ("availability_matrix", avail_matrix_path),
    ...
    # Stage 2: Load data
    stage_start = time.time()
    avail_df = pd.read_csv(avail_matrix_path)
    # Normalise zone column — may be 'zone' or 'zone_name'
    if "zone_name" in avail_df.columns:
        avail_df = avail_df.rename(columns={"zone_name": "zone"})
    avail_df = avail_df.set_index("zone")

    # Extract zone areas from the availability matrix (added by exclusion rules)
    if "area_km2" not in avail_df.columns:
        raise ValueError(
            "availability_matrix must include 'area_km2' column. "
            "Ensure per-technology exclusion rules add zone areas."
        )
    zone_areas = avail_df.pop("area_km2")  # remove from df, keep as Series

    tech_cols = list(avail_df.columns)
    logger.info(
        f"  Availability matrix: {avail_df.shape[0]} zones, "
        f"technologies: {tech_cols}, area_km2: present"
    )
    ...
    onshore_df = calculate_onshore_potential(
        avail_df, zone_areas, config, nuclear_config, hydrogen_config
    )
    ...
    log_execution_summary(
        logger,
        script_name="Calculate Technical Potential",
    ...
    logger.info("=" * 80)
    logger.info("CALCULATE TECHNICAL POTENTIAL — COMPLETE")
    logger.info("=" * 80)
